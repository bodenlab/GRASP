...
#'
#'@param asrStructure the named list returned by \code{\link{runASR}} or \code{\link{loadASR}}
  ...
  if (!is.null(asrStructure)) {
    ...
    if (is.data.frame(df)) {
      ...
      if (!all(colnames %in% cols)) {
        stop(paste("The dataframe provided as asrStructure$", dataframeName, " is not correctly formatted. See read_fasta()", sep=""))
      }
    } else {
    ...
    if(is.null(dataframe)) {
      stop(paste("You have not provided an asrStructure or specified ", dataframeName, ". See read_fasta()", sep=""))
    } else {
      if (is.data.frame(dataframe)) {
        cols <- colnames(dataframe)
        if (!all(colnames %in% cols)) {
          stop(paste("The dataframe provided as ", dataframeName, " is not correctly formatted. See read_fasta()", sep=""))
        }
      } else {
